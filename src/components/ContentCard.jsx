...
function ContentCard({ content, ...rest }) {
  ...
  const handleRenameContent = (e) => {
    ...
    content?.id
      ? renameDirectory(content.id, contentName)(dispatch)
      : renameFile(content.uploadId, contentName)(fileDispatch);
    uiDispatch({ type: "REMOVE_ACTIVE_RENAMING" });
    setcontentName("");
  };
  ...
  //     return;
  //   }
  //   renameFile(content?.uploadId, contentName)(dispatch);
  // };

  // Detect click outside to remove active content
  useEffect(() => {
